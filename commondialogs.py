...
class MapVariableDialog(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.ButtonSizer = self.CreateButtonSizer(wx.OK|wx.CANCEL)
        self.Bind(wx.EVT_BUTTON, self.OnOK, id=wx.ID_OK)#self.ButtonSizer.GetAffirmativeButton().GetId())
...
class CreateNodeDialog(wx.Dialog):
    ...
    def OnProfileChoice(self, event):
        if self.Profile.GetStringSelection() == "Other":
            dialog = wx.FileDialog(self, "Choose a file", self.Directory, "",  "OD Profile files (*.prf)|*.prf|All files|*.*", wx.FD_OPEN|wx.FD_CHANGE_DIR)
            dialog.ShowModal()
